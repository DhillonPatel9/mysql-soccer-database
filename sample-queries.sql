...
SELECT p.player_id, p.first_name, p.last_name, SUM(goal_assists) AS total_assists
FROM player p
JOIN player_match_stats pm ON p.player_id = pm.player_id
...
SELECT winner, COUNT(DISTINCT m.match_id) AS win_count, 
	ROUND((COUNT(DISTINCT m.match_id) / 
(SELECT COUNT(*) FROM matches WHERE season = 'spring' AND match_date LIKE '2024%')
	) * 100, 2) AS win_rate
FROM matches m
...
SELECT DISTINCT CONCAT(p.first_name, ' ', p.last_name) AS player_name, COUNT(player_position) AS player_count
FROM player p
